...
import { HAZARDS, type PlanContext, type Template } from '../api';
import { KCard, KField, KInput, KSelect, KTextarea } from '../krds';
    ...
    <>
      <KCard title="HWPX 문서 템플릿" desc="스타일 분석 결과를 목차·초안 생성 규칙과 내보내기에 적용합니다" right={<Link to="/plan/templates" className="tiny">템플릿 관리 →</Link>}>
        <div style={{ display: 'flex', gap: 12, overflowX: 'auto' }}>
          {templates.map((t) => (
            <button type="button" key={t.id} className={`tpl-card${c.templateId === t.id ? ' sel' : ''}`} aria-pressed={c.templateId === t.id} onClick={() => onChange({ ...c, templateId: t.id, outlineNumbering: t.levels.map((l) => l.bullet).filter(Boolean).join(' ') })}>
