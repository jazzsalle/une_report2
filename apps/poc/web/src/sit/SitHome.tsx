...
import { del, get, ago, HAZARDS, MODES, MODE_LABEL, type Exercise, type ExMode, type PlanSummary } from '../api';
...
export function SitHome() {
  ...
  useEffect(load, []);
  useEffect(() => { setPage(1); }, [q, hazard, status, mode, sort, pageSize]);
  const filtered = useMemo(() => {
    const f = list.filter((e) => (!q || e.title.includes(q)) && (!hazard || e.hazardType === hazard) && (!status || e.status === status) && (!mode || (e.mode ?? '안전한국훈련') === mode));
    const val = (e: Exercise): string => e[sort.key] ?? '';
    return [...f].sort((a, b) => val(a).localeCompare(val(b), 'ko') * (sort.dir === 'asc' ? 1 : -1));
  }, [list, q, hazard, status, mode, sort]);
  ...
  return (
    <>
      <HeroBand title="상황을 등록하고 SOP·전파·상황일지까지 한 번에 진행해보세요!" moreTo="/sit/new" moreLabel="상황 생성">
        <HeroNewCard label="새 상황 생성" onClick={() => nav('/sit/new')} />
        {recent.map((e) => {
          const st = EX_STATUS[e.status] ?? EX_STATUS.DRAFT;
          return <HeroCard key={e.id} hazard={e.hazardType} title={e.title} rowLabel={`${MODE_LABEL[e.mode ?? '안전한국훈련']} · 경보 ${e.alertLevel}`} rowValue={st.label} rowDot={st.dot} foot={[e.createdBy, ago(e.updatedAt)]} onClick={() => nav(`/sit/${e.id}`)} titleAttr={`${e.title} — 대시보드 열기`} />;
        })}
        {!list.length && <p className="hero-empty">등록된 상황이 없습니다. [새 상황 생성]으로 시작하거나, 아래에서 계획서로 훈련을 시작할 수 있습니다.</p>}
      </HeroBand>
      <div className="wrap" style={{ paddingTop: 8, paddingBottom: 24 }}>
        <div className="stack" style={{ gap: 24 }}>
          <KCard title={<>상황 전체 목록 <span className="dim" style={{ fontWeight: 400, fontSize: 15 }}>({filtered.length}/{list.length})</span></>} right={
            <div className="row">
              <KInput className="search" placeholder="상황명 검색" value={q} onChange={(e) => setQ(e.target.value)} style={{ width: 180 }} aria-label="상황명 검색" />
              <KSelect value={mode} onChange={(e) => setMode(e.target.value as '' | ExMode)} style={{ width: 120 }} aria-label="모드"><option value="">모드 전체</option>{MODES.map((m) => <option key={m} value={m}>{m}</option>)}</KSelect>
            ...
            <KTable caption="훈련상황 목록" widths={['40px', undefined, '7%', '12%', '8%', '8%', '9%', '7%', '11%', '9%', '11%']} emptyText="등록된 상황이 없습니다. [새 상황 생성]으로 시작하세요."
              head={[<span className="sr-only">선택</span>, th('상황명', 'title'), '모드', th('재난유형', 'hazardType'), '경보', '훈련단계', th('상태', 'status'), th('생성자', 'createdBy'), th('생성 일시', 'createdAt'), th('수정 일시', 'updatedAt'), '연동 계획서']}
              rows={pageRows.map((e) => {
                ...
                return [
                  ...
                  <KBadge tone={(e.mode ?? '안전한국훈련') === '실제상황' ? 'danger' : e.mode === '도상훈련' ? 'light-gray' : 'light-primary'}>{MODE_LABEL[e.mode ?? '안전한국훈련']}</KBadge>,
      ...
      {confirmDel && (
        <KModal title="삭제하기" onClose={() => setConfirmDel(null)}>
          <p style={{ fontSize: 15 }}>선택한 상황 {confirmDel.length}개를 휴지통으로 옮깁니다. 휴지통에서 30일 안에 복원할 수 있습니다.</p>
