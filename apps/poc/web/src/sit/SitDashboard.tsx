...
export function SitDashboard() {
  ...
  const donut = (p: number) => { const r = 34, c = 2 * Math.PI * r; return <svg width={90} height={90} role="img" aria-label={`진행률 ${p}%`}><circle cx={45} cy={45} r={r} stroke="#cdd1d5" strokeWidth={12} fill="none" /><circle cx={45} cy={45} r={r} stroke={C.green} strokeWidth={12} fill="none" strokeDasharray={`${(p / 100) * c} ${c}`} transform="rotate(-90 45 45)" /><text x={45} y={50} textAnchor="middle" fontSize={16} fontWeight={800}>{p}%</text></svg>; };
  return (
    <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, overflow: 'auto', height: '100%', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
        {FLOW.map((f, i) => <span key={f} style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span className={`chip ${i === stepIdx ? 'now' : i < stepIdx ? 'done' : 'todo'}`} style={{ cursor: 'default' }} aria-current={i === stepIdx ? 'step' : undefined}>{i < stepIdx ? '✓ ' : ''}{f}</span>{i < FLOW.length - 1 && <span style={{ color: C.muted, opacity: .5 }}>›</span>}</span>)}
        <div style={{ flex: 1 }} />
