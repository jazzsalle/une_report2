...
export interface SopNode { id: string; type: NodeType; title: string; dept?: string; assignee?: string; priority?: string; due?: string; channels?: string[]; tasks?: string[]; logRules?: string[];
  // 범용화 ② (2026-08-23): 매뉴얼 조치카드 출처 — 코드·협업기능·주관/지원/협업·연계코드·단계·원문 발췌
  code?: string; coop?: string; lead?: string; support?: string[]; partner?: string[]; linkedCodes?: string[]; stage?: string; sourceRef?: { doc: string; score: number; excerpt: string } }
...
// ── 범용화 ① (2026-08-23): 모드 · 대응 단계 · 위기경보 · 상황판단회의 · 기관 설정 ──
export type ExMode = '실제상황' | '안전한국훈련' | '도상훈련';
export type ExStage = '징후감지' | '초기대응' | '비상1' | '비상2·3' | '수습복구';
export const MODES: ExMode[] = ['실제상황', '안전한국훈련', '도상훈련'];
export const STAGES: ExStage[] = ['징후감지', '초기대응', '비상1', '비상2·3', '수습복구'];
export const ALERTS = ['관심', '주의', '경계', '심각'];
/** 위기경보 색 — 표준매뉴얼 Blue/Yellow/Orange/Red */
export const ALERT_COLOR: Record<string, { bg: string; fg: string }> = { 관심: { bg: '#e8f1ff', fg: '#0b50d0' }, 주의: { bg: '#fff4c2', fg: '#8a5a00' }, 경계: { bg: '#ffe0cc', fg: '#c2410c' }, 심각: { bg: '#fde2e1', fg: '#b91c1c' } };
export const MODE_LABEL: Record<ExMode, string> = { 실제상황: '실제', 안전한국훈련: '훈련', 도상훈련: '도상' };
export interface Meeting { id: string; exerciseId: string; at: string; chair: string; attendees: string[]; agenda: string; decisions: { alertLevel?: string; stage?: ExStage; evacuation?: boolean; cbs?: boolean; other?: string }; memo?: string; by?: string }
// ── 범용화 ② (2026-08-23): 매뉴얼 지식화 — 조치카드 · SOP 템플릿 ──
export type ManualStage = ExStage;
export const MANUAL_STAGES: ManualStage[] = ['징후감지', '초기대응', '비상1', '비상2·3', '수습복구'];
export interface Manual { id: string; name: string; hazard: string; tier: '표준' | '실무' | '현장조치'; org: string; version?: string; queryPrefix: string; docFilter: string; docNames: string[]; extractedAt?: string; cardCount: number; reviewedCount?: number; templateCount?: number; extracting?: boolean; lastRun?: { queries: number; elapsedMs: number; skippedDocs: string[] }; createdAt: string; updatedAt: string }
export interface ActionCard { id: string; manualId: string; manualName?: string; code: string; coop: string; seq: [number, number]; title: string; content: string; lead: string; support: string[]; partner: string[]; linkedCodes: { code: string; title: string }[]; checklist: string[]; stage: ManualStage | null; truncated: boolean; reviewed: boolean; note?: string; sourceRef: { doc: string; score: number; excerpt: string; query: string } }
export interface SopTemplateSummary { id: string; name: string; hazard: string; manualId: string; manualName?: string; stages: ManualStage[]; coops: string[]; nodes: number; createdAt: string; createdBy?: string }
...
/** 협업기능 원문자 ①~⑬ → 번호 */
export const coopIndex = (c: string) => { const i = '①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳'.indexOf(c); return i < 0 ? 99 : i; };
export interface PendingWarning { id: string; at: string; change: '발표' | '해제'; kind: string; level: string; regions: string }
