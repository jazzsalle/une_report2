...
| ADR-50 | CC-410 실 UNI 계약 결속과 매핑 재작성 (uni-sop-2) | ACCEPTED | [ADR-50-cc410-uni-measured-contract-binding.md](ADR-50-cc410-uni-measured-contract-binding.md) | knowledgeStatus 경로·상태 어휘 재작성(라이브에 GET /documents/{id} 없음), UNI 삭제 재인증 401 확인, doc_ids 범위 지정 요청(CR-UNI-008), 유형 코드 표 수령, SSE 재접속 수단(id:/retry:), UNI 422의 비밀번호 에코 수정 |
| ADR-51 | UNI 호스트 이전 (221.147.100.161:8000 → 10.20.10.101:8088) | ACCEPTED | [ADR-51-uni-host-relocation.md](ADR-51-uni-host-relocation.md) | 어댑터 env 키 불일치(UNI_* vs UNE_UNI_*) 해소, OB-13(a) knowledgeStatus 경로 부재는 신 호스트에도 그대로, UNI_PASSWORD 교체, TLS 종단, 업로드·검색·SOP 스트림의 신 호스트 재확인 |
