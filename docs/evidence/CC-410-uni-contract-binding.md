...
- **정정 2026-08-18 (ADR-51): UNI 호스트가 `http://10.20.10.101:8088`으로 이전했다.**
  이 문서의 주소는 **고치지 않는다** — 아래 측정은 전부 옛 주소에서 일어났고,
  주소를 바꿔 적으면 하지 않은 곳에서 측정했다고 감사에 남는다. 신 호스트에서
  재확인한 것은 `/openapi.json`(같은 `UNI RAG System` 1.1.0, 26경로)과
  `POST /auth/login` 200(`token`+`user`, 같은 모양) 둘뿐이다. **§2~§6의 나머지
  실측은 신 호스트에서 재확인되지 않았다.**
