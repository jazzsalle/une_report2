...
>
> ADR-49(2026-08-14)로 셋이 정리됐다 — AV는 두지 않고(OB-15 닫힘), 채널은
> 시뮬레이션으로 가고(OB-06 차단 아님), 인도 환경은 고정 IP를 받고 정한다.
> **수용한 위험은 §7 두 번째 표에 적혀 있다.** 차단이 아니라는 것과 위험이
> 없다는 것은 다르다.
...
- `UNE_KNOWLEDGE_ALLOW_SCAN_PENDING` — **`true`로 배포한다**(ADR-49 D1).
  AV를 두지 않기로 했으므로 검사 없는 파일 등록을 수용한다. 켜져 있다는 사실
  자체가 그 수용의 기록이므로 코드 기본값은 `false`로 남아 있다 — 빠뜨리면
  지식문서 등록이 막힌다.
...
### 차단이 아니지만 알고 배포해야 하는 것 (ADR-49)

| 항목 | 수용한 위험 |
|---|---|
| **OB-15** (닫힘) | AV 엔진을 두지 않기로 했다. **지식문서 파일은 어떤 악성코드 검사도 거치지 않고 UNI에 색인된다** — UNE는 크기·SHA-256·선언 MIME만 본다. `UNE_KNOWLEDGE_ALLOW_SCAN_PENDING=true`를 명시해야 하며, 빠뜨리면 등록이 fail-closed로 막힌다. `scan_status`는 PENDING에 머문다 — 검사했다고 적지 않는다 |
| **OB-06** (차단 아님) | 시뮬레이션 채널만으로 진행한다. **전파는 아무에게도 배달되지 않는다.** 현장 담당자는 로그인 경로로만 임무를 받는다. 시뮬레이션 여부는 전파 상태에 드러난다(ADR-41 D2) |
| **OB-14** (기준 확정) | 최종 인도 환경은 **고정 IP 확보 후** 결정한다. 트레이스 수집기와 오브젝트 저장소 백업이 여기에 딸려 있다 |
