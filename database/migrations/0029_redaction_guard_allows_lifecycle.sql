...
-- ===========================================================================
-- 이 트리거는 막지 않는다. 막는 것은 `ck_provider_job_outcome_shape`(0028)와
-- 권한 경계다: `une_app`은 provider_job에 UPDATE가 없고(0023 §5), `une_worker`는
-- 0028 §6의 정책이 미종결 행만 보여주므로 종결된 잡을 다시 집을 수 없다.
-- 상태 자체의 단방향성은 별도 항목으로 남긴다 — 지금은 도달 경로가 없다.
